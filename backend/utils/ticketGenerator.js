...
const generateTicketNumber = async (designation, traineeType) => {
  return new Promise((resolve, reject) => {
    console.log(
      `Queuing ticket generation for designation: ${designation} in ${traineeType}`
    );

    if (!traineeType) {
      reject(
        new Error("Trainee type is required to generate a ticket number.")
      );
      return;
    }
...
const processTicketQueue = async () => {
  ...
  while (ticketGenerationQueue.length > 0) {
    const { designation, traineeType, resolve, reject } =
      ticketGenerationQueue.shift();

    try {
      const ticketNumber = await generateTicketNumberInternal(
        designation,
        traineeType
      );
...
const generateTicketNumberInternal = async (designation, traineeType) => {
  return new Promise((resolve, reject) => {
    console.log(
      `Generating ticket number for designation: ${designation} in ${traineeType}`
    );

    let prefix;
    // Special handling for WTC/Non-Railway designations
    ...
    db.all(query, [`${prefix}`], (err, rows) => {
      if (err) {
        console.error(
          `Error fetching ticket numbers for ${designation}:`,
          err.message
        );
        reject(err);
        return;
      ...
      if (
        generatedNumbers[prefixKey] &&
        generatedNumbers[prefixKey] > maxNumber
      ) {
      ...
      console.log(
        `Generated new ticket number: ${newTicketNumber} (prefix: ${prefix}, max found: ${maxNumber})`
      );
