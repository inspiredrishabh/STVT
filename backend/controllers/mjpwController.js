...
class MjpWController {
    ...
    async updateScoreByTicketNumber(req, res) {
        try {
            ...
            const updatedScore = await this.mjpWModel.updateByTicketNumber(ticketNumber, updatedData);
    ...
    async getScoreWithCandidateDetails(req, res) {
        try {
            const { ticketNumber } = req.params;
            const scoreWithDetails = await this.mjpWModel.getScoreWithCandidateDetails(ticketNumber);
