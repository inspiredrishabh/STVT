...
class MjrCwController {
    constructor(mjrCwModel, stcModel) {
        this.mjrCwModel = mjrCwModel;
    ...
    async createScore(req, res) {
        try {
            ...
            if (scoreData.ticket_no || scoreData.ticketNumber) {
                ...
                if (stcCandidate.designation !== 'MJR-C&W') {
                    return res.status(400).json({
                        success: false,
                        message: `Candidate designation is ${stcCandidate.designation}, not MJR-C&W`
                    });
                }
            }

            const newScore = await this.mjrCwModel.create(scoreData);
            res.status(201).json({
                success: true,
                message: 'MJR-C&W Score created successfully',
                data: newScore
            });
        } catch (error) {
            console.error('Error creating MJR-C&W score:', error);
            res.status(500).json({
                success: false,
                message: error.message || 'Failed to create MJR-C&W score'
            });
        }
    }
    ...
    async getScores(req, res) {
        try {
            const scores = await this.mjrCwModel.getAll();
            res.status(200).json({
                success: true,
                message: 'MJR-C&W Scores retrieved successfully',
                data: scores,
                count: scores.length
            });
        } catch (error) {
            console.error('Error getting MJR-C&W scores:', error);
            res.status(500).json({
                success: false,
                message: 'Failed to retrieve MJR-C&W scores'
            });
        }
    }
    ...
    async getScoreByTicketNumber(req, res) {
        try {
            ...
            const score = await this.mjrCwModel.getByTicketNumber(ticketNumber);
            
            if (!score) {
                return res.status(404).json({
                    success: false,
                    message: 'MJR-C&W Score not found'
                });
            }

            res.status(200).json({
                success: true,
                message: 'MJR-C&W Score retrieved successfully',
                data: score
            });
        } catch (error) {
            console.error('Error getting MJR-C&W score:', error);
            res.status(500).json({
                success: false,
                message: 'Failed to retrieve MJR-C&W score'
            });
        }
    }
    ...
    async updateScoreByTicketNumber(req, res) {
        try {
            ...
            const existingScore = await this.mjrCwModel.getByTicketNumber(ticketNumber);
            if (!existingScore) {
                return res.status(404).json({
                    success: false,
                    message: 'MJR-C&W Score not found'
            ...
            const updatedScore = await this.mjrCwModel.updateByTicketNumber(ticketNumber, updatedData);
            
            res.status(200).json({
                success: true,
                message: 'MJR-C&W Score updated successfully',
                data: updatedScore
            });
        } catch (error) {
            console.error('Error updating MJR-C&W score:', error);
            res.status(500).json({
                success: false,
                message: error.message || 'Failed to update MJR-C&W score'
            });
        }
    }
    ...
    async deleteScoreByTicketNumber(req, res) {
        try {
            ...
            const deleted = await this.mjrCwModel.deleteByTicketNumber(ticketNumber);
            
            res.status(200).json({
                success: true,
                message: 'MJR-C&W Score deleted successfully'
            });
        } catch (error) {
            console.error('Error deleting MJR-C&W score:', error);
            res.status(500).json({
                success: false,
                message: 'Failed to delete MJR-C&W score'
            });
        }
    }
    ...
    async updatePaperMarks(req, res) {
        try {
            ...
            const updated = await this.mjrCwModel.updatePaperMarks(ticketNumber, paperCode, marks);
    ...
    async getMarksSummary(req, res) {
        try {
            const { ticketNumber } = req.params;
            const summary = await this.mjrCwModel.getMarksSummary(ticketNumber);
            ...
            res.status(200).json({
                success: true,
                message: 'MJR-C&W Marks summary retrieved successfully',
                data: summary
            });
        } catch (error) {
    ...
    async getScoresWithCandidateDetails(req, res) {
        try {
            const scoresWithDetails = await this.mjrCwModel.getScoresWithCandidateDetails();
            res.status(200).json({
                success: true,
                message: 'MJR-C&W Scores with candidate details retrieved successfully',
    ...
    async getScoreWithCandidateDetails(req, res) {
        try {
            const { ticketNumber } = req.params;
            const scoreWithDetails = await this.mjrCwModel.getScoreWithCandidateDetails(ticketNumber);
            
            if (!scoreWithDetails) {
                return res.status(404).json({
                    success: false,
                    message: 'MJR-C&W Candidate or Score not found'
                });
            }

            res.status(200).json({
                success: true,
                message: 'MJR-C&W Score with candidate details retrieved successfully',
    ...
    async getSessionMarks(req, res) {
        try {
            ...
            res.status(200).json({
                success: true,
                message: `MJR-C&W Session ${session} marks retrieved successfully`,
                data: {
    ...
    async getSessionAnalysis(req, res) {
        try {
            ...
            const allScores = await this.mjrCwModel.getAll();
            
            if (allScores.length === 0) {
                return res.status(404).json({
                    success: false,
                    message: 'No MJR-C&W scores found'
                });
            }
            ...
            res.status(200).json({
                success: true,
                message: `MJR-C&W Session ${session} analysis retrieved successfully`,
                data: analysis
            });
        } catch (error) {
    ...
    async getCourseAnalysis(req, res) {
        try {
            ...
            res.status(200).json({
                success: true,
                message: 'MJR-C&W Course analysis retrieved successfully',
                data: analysis
            });
        } catch (error) {
    ...
    async bulkUploadScores(req, res) {
        try {
            ...
            for (let i = 0; i < scores.length; i++) {
                try {
                    ...
                    if (ticketNo) {
                        ...
                        if (stcCandidate.designation !== 'MJR-C&W') {
                            throw new Error(`Candidate designation is ${stcCandidate.designation}, not MJR-C&W`);
                        }
                    }
                    
                    const newScore = await this.mjrCwModel.create(score);
            ...
            console.error('Error in bulk upload MJR-C&W scores:', error);
            res.status(500).json({
                success: false,
    ...
    async upsertScore(req, res) {
        try {
            ...
            if (existingScore) {
                ...
                result = await this.mjrCwModel.updateByTicketNumber(ticketNumber, scoreData);
                message = 'MJR-C&W Score updated successfully';
            } else {
                // Create new score
                result = await this.mjrCwModel.create(scoreData);
                message = 'MJR-C&W Score created successfully';
            }
            ...
            console.error('Error in upsert MJR-C&W score:', error);
            res.status(500).json({
                success: false,
                message: error.message || 'Failed to upsert MJR-C&W score'
            });
        }
    }
...
module.exports = MjrCwController;