...
class WtcController {
  ...
  async updateCandidateByTicketNumber(req, res) {
    try {
      ...
      // If ticketNumber is being updated, ensure ticket_no is set in filteredData
      if (updatedData.ticketNumber) {
        filteredData.ticket_no = updatedData.ticketNumber;
      }

      // Check if candidate exists
      const existingCandidate = await this.wtcModel.getByTicketNumber(
        ticketNumber
      );
      ...
      const updatedCandidate = await this.wtcModel.updateByTicketNumber(
        ticketNumber,
        filteredData
      );
