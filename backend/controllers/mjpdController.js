...
class MjpDController {
    ...
    async getScoresWithCandidateDetails(req, res) {
        try {
            ...
            console.error('Error getting MJP - D scores with candidate details:', error);
            res.status(500).json({
                success: false,
                message: 'Failed to retrieve MJP - D scores with candidate details'
            });
        }
    }
    ...
    async getScoreWithCandidateDetails(req, res) {
        try {
            ...
            if (!scoreWithDetails) {
                return res.status(404).json({
                    success: false,
                    message: 'MJP - D Score with candidate details not found'
                });
            }
            ...
            console.error('Error getting MJP - D score with candidate details:', error);
            res.status(500).json({
                success: false,
                message: 'Failed to retrieve MJP - D score with candidate details'
            });
        }
    }
