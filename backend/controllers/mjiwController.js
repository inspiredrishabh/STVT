...
class MjiWController {
    ...
    async getScoresWithCandidateDetails(req, res) {
        try {
            ...
            console.error('Error getting MJI - W scores with candidate details:', error);
            res.status(500).json({
                success: false,
                message: 'Failed to retrieve MJI - W scores with candidate details'
            });
        }
    }
    ...
    async getScoreWithCandidateDetails(req, res) {
        try {
            ...
            if (!scoreWithDetails) {
                return res.status(404).json({
                    success: false,
                    message: 'MJI - W Score with candidate details not found'
                });
            }
            ...
            console.error('Error getting MJI - W score with candidate details:', error);
            res.status(500).json({
                success: false,
                message: 'Failed to retrieve MJI - W score with candidate details'
            });
        }
    }
