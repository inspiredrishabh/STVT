...
class MjrDController {
    constructor(mjrDModel, stcModel) {
        this.mjrDModel = mjrDModel;
    ...
    async createScore(req, res) {
        try {
            ...
            if (scoreData.ticket_no || scoreData.ticketNumber) {
                ...
                if (stcCandidate.designation !== 'MJR-D') {
                    return res.status(400).json({
                        success: false,
                        message: `Candidate designation is ${stcCandidate.designation}, not MJR-D`
                    });
                }
            }

            const newScore = await this.mjrDModel.create(scoreData);
            res.status(201).json({
                success: true,
                message: 'MJR-D Score created successfully',
                data: newScore
            });
        } catch (error) {
            console.error('Error creating MJR-D score:', error);
            res.status(500).json({
                success: false,
                message: error.message || 'Failed to create MJR-D score'
            });
        }
    }
    ...
    async getScores(req, res) {
        try {
            const scores = await this.mjrDModel.getAll();
            res.status(200).json({
                success: true,
                message: 'MJR-D Scores retrieved successfully',
                data: scores,
                count: scores.length
            });
        } catch (error) {
            console.error('Error getting MJR-D scores:', error);
            res.status(500).json({
                success: false,
                message: 'Failed to retrieve MJR-D scores'
            });
        }
    }
    ...
    async getScoreByTicketNumber(req, res) {
        try {
            ...
            const score = await this.mjrDModel.getByTicketNumber(ticketNumber);
            
            if (!score) {
                return res.status(404).json({
                    success: false,
                    message: 'MJR-D Score not found'
                });
            }

            res.status(200).json({
                success: true,
                message: 'MJR-D Score retrieved successfully',
                data: score
            });
        } catch (error) {
            console.error('Error getting MJR-D score:', error);
            res.status(500).json({
                success: false,
                message: 'Failed to retrieve MJR-D score'
            });
        }
    }
    ...
    async updateScoreByTicketNumber(req, res) {
        try {
            ...
            const existingScore = await this.mjrDModel.getByTicketNumber(ticketNumber);
            if (!existingScore) {
                return res.status(404).json({
                    success: false,
                    message: 'MJR-D Score not found'
            ...
            const updatedScore = await this.mjrDModel.updateByTicketNumber(ticketNumber, updatedData);
            
            res.status(200).json({
                success: true,
                message: 'MJR-D Score updated successfully',
                data: updatedScore
            });
        } catch (error) {
            console.error('Error updating MJR-D score:', error);
            res.status(500).json({
                success: false,
                message: error.message || 'Failed to update MJR-D score'
            });
        }
    }
    ...
    async deleteScoreByTicketNumber(req, res) {
        try {
            ...
            const deleted = await this.mjrDModel.deleteByTicketNumber(ticketNumber);
            
            res.status(200).json({
                success: true,
                message: 'MJR-D Score deleted successfully'
            });
        } catch (error) {
            console.error('Error deleting MJR-D score:', error);
            res.status(500).json({
                success: false,
                message: 'Failed to delete MJR-D score'
            });
        }
    }
    ...
    async updatePaperMarks(req, res) {
        try {
            ...
            const updated = await this.mjrDModel.updatePaperMarks(ticketNumber, paperCode, marks);
    ...
    async getMarksSummary(req, res) {
        try {
            const { ticketNumber } = req.params;
            const summary = await this.mjrDModel.getMarksSummary(ticketNumber);
            ...
            res.status(200).json({
                success: true,
                message: 'MJR-D Marks summary retrieved successfully',
                data: summary
            });
        } catch (error) {
    ...
    async getScoresWithCandidateDetails(req, res) {
        try {
            const scoresWithDetails = await this.mjrDModel.getScoresWithCandidateDetails();
            res.status(200).json({
                success: true,
                message: 'MJR-D Scores with candidate details retrieved successfully',
    ...
    async getScoreWithCandidateDetails(req, res) {
        try {
            const { ticketNumber } = req.params;
            const scoreWithDetails = await this.mjrDModel.getScoreWithCandidateDetails(ticketNumber);
            
            if (!scoreWithDetails) {
                return res.status(404).json({
                    success: false,
                    message: 'MJR-D Candidate or Score not found'
                });
            }

            res.status(200).json({
                success: true,
                message: 'MJR-D Score with candidate details retrieved successfully',
    ...
    async getSessionMarks(req, res) {
        try {
            ...
            res.status(200).json({
                success: true,
                message: `MJR-D Session ${session} marks retrieved successfully`,
                data: {
    ...
    async getSessionAnalysis(req, res) {
        try {
            ...
            const allScores = await this.mjrDModel.getAll();
            
            if (allScores.length === 0) {
                return res.status(404).json({
                    success: false,
                    message: 'No MJR-D scores found'
                });
            }
            ...
            res.status(200).json({
                success: true,
                message: `MJR-D Session ${session} analysis retrieved successfully`,
                data: analysis
            });
        } catch (error) {
    ...
    async getCourseAnalysis(req, res) {
        try {
            ...
            res.status(200).json({
                success: true,
                message: 'MJR-D Course analysis retrieved successfully',
                data: analysis
            });
        } catch (error) {
    ...
    async bulkUploadScores(req, res) {
        try {
            ...
            for (let i = 0; i < scores.length; i++) {
                try {
                    ...
                    if (ticketNo) {
                        ...
                        if (stcCandidate.designation !== 'MJR-D') {
                            throw new Error(`Candidate designation is ${stcCandidate.designation}, not MJR-D`);
                        }
                    }
                    
                    const newScore = await this.mjrDModel.create(score);
            ...
            console.error('Error in bulk upload MJR-D scores:', error);
            res.status(500).json({
                success: false,
    ...
    async upsertScore(req, res) {
        try {
            ...
            if (existingScore) {
                ...
                result = await this.mjrDModel.updateByTicketNumber(ticketNumber, scoreData);
                message = 'MJR-D Score updated successfully';
            } else {
                // Create new score
                result = await this.mjrDModel.create(scoreData);
                message = 'MJR-D Score created successfully';
            }
            ...
            console.error('Error in upsert MJR-D score:', error);
            res.status(500).json({
                success: false,
                message: error.message || 'Failed to upsert MJR-D score'
            });
        }
    }
