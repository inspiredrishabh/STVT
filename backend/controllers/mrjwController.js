...
class MjrWController {
    constructor(mjrWModel, stcModel) {
    ...
    async createScore(req, res) {
        try {
            ...
            if (scoreData.ticket_no || scoreData.ticketNumber) {
                ...
                if (stcCandidate.designation !== 'MJR-W') {
                    return res.status(400).json({
                        success: false,
                        message: `Candidate designation is ${stcCandidate.designation}, not MJR-W`
                    });
                }
            }
    ...
    async updatePaperMarks(req, res) {
        try {
            ...
            const updated = await this.mjrWModel.updatePaperMarks(ticketNumber, paperCode, marks);
    ...
    async getMarksSummary(req, res) {
        try {
            const { ticketNumber } = req.params;
            const summary = await this.mjrWModel.getMarksSummary(ticketNumber);
            ...
            res.status(200).json({
                success: true,
                message: 'MJR-W Marks summary retrieved successfully',
                data: summary
            });
        } catch (error) {
    ...
    async getScoreWithCandidateDetails(req, res) {
        try {
            ...
            if (!scoreWithDetails) {
                return res.status(404).json({
                    success: false,
                    message: 'MJR-W Candidate or Score not found'
                });
            }
    ...
    async getSessionMarks(req, res) {
        try {
            ...
            res.status(200).json({
                success: true,
                message: `MJR-W Session ${session} marks retrieved successfully`,
                data: {
    ...
    async getSessionAnalysis(req, res) {
        try {
            ...
            const allScores = await this.mjrWModel.getAll();
            
            if (allScores.length === 0) {
                return res.status(404).json({
                    success: false,
                    message: 'No MJR-W scores found'
                });
            }
            ...
            res.status(200).json({
                success: true,
                message: `MJR-W Session ${session} analysis retrieved successfully`,
                data: analysis
            });
        } catch (error) {
    ...
    async getCourseAnalysis(req, res) {
        try {
            ...
            res.status(200).json({
                success: true,
                message: 'MJR-W Course analysis retrieved successfully',
                data: analysis
            });
        } catch (error) {
            ...
            for (let i = 0; i < scores.length; i++) {
                try {
                    ...
                    if (ticketNo) {
                        ...
                        if (stcCandidate.designation !== 'MJR-W') {
                            throw new Error(`Candidate designation is ${stcCandidate.designation}, not MJR-W`);
                        }
                    }
