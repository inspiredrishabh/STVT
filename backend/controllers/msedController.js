...
class MseDController {
    constructor(mseDModel, stcModel) {
    ...
    async updatePaperMarks(req, res) {
        try {
            ...
            const updated = await this.mseDModel.updatePaperMarks(ticketNumber, paperCode, marks);
    ...
    async getMarksSummary(req, res) {
        try {
            const { ticketNumber } = req.params;
            const summary = await this.mseDModel.getMarksSummary(ticketNumber);
            ...
            res.status(200).json({
                success: true,
                message: 'MSE-D Marks summary retrieved successfully',
                data: summary
            });
        } catch (error) {
    ...
    async getScoreWithCandidateDetails(req, res) {
        try {
            ...
            if (!scoreWithDetails) {
                return res.status(404).json({
                    success: false,
                    message: 'MSE-D Candidate or Score not found'
                });
            }
    ...
    async getSessionMarks(req, res) {
        try {
            ...
            res.status(200).json({
                success: true,
                message: `MSE-D Session ${session} marks retrieved successfully`,
                data: {
    ...
    async getSessionAnalysis(req, res) {
        try {
            ...
            const allScores = await this.mseDModel.getAll();
            
            if (allScores.length === 0) {
                return res.status(404).json({
                    success: false,
                    message: 'No MSE-D scores found'
                });
            }
            ...
            res.status(200).json({
                success: true,
                message: `MSE-D Session ${session} analysis retrieved successfully`,
                data: analysis
            });
        } catch (error) {
    ...
    async getCourseAnalysis(req, res) {
        try {
            ...
            res.status(200).json({
                success: true,
                message: 'MSE-D Course analysis retrieved successfully',
                data: analysis
            });
        } catch (error) {
            ...
            for (let i = 0; i < scores.length; i++) {
                try {
                    ...
                    if (ticketNo) {
                        ...
                        if (stcCandidate.designation !== 'MSE-D') {
                            throw new Error(`Candidate designation is ${stcCandidate.designation}, not MSE-D`);
                        }
                    }
