...
class MjiCwModel {
    ...
    createTable() {
        db.run(`CREATE TABLE IF NOT EXISTS ${this.tableName} (
            ...
            -- Main identifier, now a foreign key
            ...
            FOREIGN KEY (ticket_no) REFERENCES stc_candidates(ticket_no) ON DELETE CASCADE ON UPDATE CASCADE
        )`);
    }
            ...
            db.run(sql, [
                ...
                if (err) {
                    if (err.message.includes('UNIQUE constraint failed')) {
                        ...
                        reject(new Error('Ticket number does not exist in stc_candidates table. Please create the candidate first.'));
                    } else {
                        reject(err);
                    }
                } else {
