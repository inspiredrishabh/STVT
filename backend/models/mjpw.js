...
class MjpWModel {
    ...
    create(scoreData) {
        return new Promise((resolve, reject) => {
            const sql = `INSERT INTO ${this.tableName} (
                ticket_no, s1p1_marks, s1p2_marks, s2p1_marks, s2pr_marks, s2int_marks
            ) VALUES (?, ?, ?, ?, ?, ?)`;
    ...
    updateByTicketNumber(ticketNumber, scoreData) {
        return new Promise((resolve, reject) => {
            const sql = `UPDATE ${this.tableName} SET 
                s1p1_marks = ?, s1p2_marks = ?, s2p1_marks = ?, s2pr_marks = ?, s2int_marks = ?,
    ...
    getMarksSummary(ticketNumber) {
        return new Promise((resolve, reject) => {
            const sql = `SELECT *,
                (s1p1_marks + s1p2_marks + s2p1_marks + s2pr_marks + s2int_marks) as total_marks,
                (s1p1_max_marks + s1p2_max_marks + s2p1_max_marks + s2pr_max_marks + s2int_max_marks) as total_max_marks
    ...
    getScoresWithCandidateDetails() {
        return new Promise((resolve, reject) => {
            const sql = `
                SELECT 
                    sc.*, 
                    mjp.s1p1_marks, mjp.s1p2_marks, mjp.s2p1_marks, mjp.s2pr_marks, mjp.s2int_marks,
                    (mjp.s1p1_marks + mjp.s1p2_marks + mjp.s2p1_marks + mjp.s2pr_marks + mjp.s2int_marks) as total_marks
