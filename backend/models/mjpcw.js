...
class MjpCwModel {
    constructor() {
        this.tableName = 'mjp_cw_candidates';
        this.createTable();
    }
    ...
    create(scoreData) {
        return new Promise((resolve, reject) => {
            const sql = `INSERT INTO ${this.tableName} (
                ticket_no, s1p1, s1p2, s1p, s2p1, s2p, s2i
            ) VALUES (?, ?, ?, ?, ?, ?, ?)`;
    ...
    getScoresWithCandidateDetails() {
        return new Promise((resolve, reject) => {
            const sql = `
                SELECT 
                    sc.*, 
                    mjp.s1p1, mjp.s1p2, mjp.s1p, mjp.s2p1, mjp.s2p, mjp.s2i
