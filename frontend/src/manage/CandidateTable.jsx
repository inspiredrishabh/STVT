...
const CandidateTable = ({ candidates, onViewDetail, onDelete, onUpdate }) => {
  ...
  const handleModalSave = async () => {
    if (onUpdate && editFormData.id) {
      await onUpdate(editFormData.id, editFormData);
    }
    ...
    <tr className="hover:bg-gray-50 transition-colors duration-200">
      <td className="px-6 py-4">
        <div className="flex items-center space-x-4">
          <img src={`http://192.168.244.85:5000/${candidate.picture}`} alt={candidate.name} className="w-12 h-12 rounded-xl object-cover shadow-md" />
          <div>
      ...
      <div className="p-8 space-y-8 overflow-y-auto flex-1">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          ...
          <EditSection
            ...
            icon={Phone}
            fields={[
              { label: "Phone Number", field: "phoneNumber", type: "tel", value: candidate.phoneNumber },
              { label: "Email Address", field: "email", type: "email", value: candidate.email },
              { label: "Emergency Contact", field: "emergencyContactNumber", type: "tel", value: candidate.emergencyContactNumber },
              { label: "Current Address", field: "currentAddress", type: "textarea", value: candidate.currentAddress },
              { label: "Permanent Address", field: "permanentAddress", type: "textarea", value: candidate.permanentAddress }
            ]}
            candidate={candidate}
            onInputChange={onInputChange}
          />
