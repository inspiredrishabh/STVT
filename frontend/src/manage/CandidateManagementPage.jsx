  ...
  async getNonRailwayCandidates() {
    try {
      ...
      if (data.success && data.data) {
        // Transform each candidate's fields from backend format to frontend format
        const transformedCandidates = data.data.map((candidate) =>
          this.transformBackendResponseToFrontend(candidate, "Non Railway")
        );

        console.log(
          "Non-Railway candidates fetched:",
          transformedCandidates.length
        );
        return transformedCandidates;
      } else {
        console.error("Failed to fetch Non-Railway candidates:", data.message);
        return [];
      }
    } catch (error) {
      console.error("Error fetching Non-Railway candidates:", error);
