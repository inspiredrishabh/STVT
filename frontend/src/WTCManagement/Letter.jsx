...
import { ArrowLeft, Download, Printer, Search, CheckSquare, Square, Filter, X, User, FileText } from 'lucide-react';
...
const Letter = () => {
    ...
    const [view, setView] = useState('trainees'); // 'trainees', 'letters'
    ...
    const generateLetter = (traineeId) => {
        const trainee = allTrainees.find(t => t.id === traineeId);
        if (!trainee) return;

        navigate(`/wtc/letter/preview?traineeId=${traineeId}&name=${encodeURIComponent(trainee.name)}&ticketNo=${trainee.ticketNo}&trade=${encodeURIComponent(trainee.trade)}&from=${trainee.from}&to=${trainee.to}`);
    };
    ...
    const exportAllToPdf = () => {
        ...
        const opt = {
            margin: [10, 10, 10, 10],
            filename: `WTC_Letters_Bulk_${new Date().toISOString().slice(0, 10)}.pdf`,
    ...
    const LetterTemplate = ({ trainee }) => {
        ...
        <div className="min-h-screen bg-gray-100 py-8 px-4">
            <div className="max-w-6xl mx-auto">
                ...
                <div className="flex justify-between items-center mb-6">
                    <div className="flex items-center space-x-4">
                        ...
                        <h1 className="text-2xl font-bold text-gray-800">
                            {view === 'trainees' ? 'Letter Management' : 'Bulk Letters Preview'}
                        </h1>
                    </div>
                                                ...
                                                <tr key={trainee.id} className="hover:bg-gray-50">
                                                    ...
                                                    <td className="p-4">
                                                        <div className="flex justify-center space-x-2">
                                                            <button
                                                                onClick={() => generateLetter(trainee.id)}
                ...
                {/* Bulk Letters View */}
                {view === 'letters' && (
                    <div ref={letterRef} className="letters-container">
