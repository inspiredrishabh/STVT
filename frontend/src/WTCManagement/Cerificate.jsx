...
import {
  ArrowLeft, Search, CheckSquare, Square, Filter, X, User, Award
} from 'lucide-react';
...
const Certificate = () => {
  ...
  const generateBulkCertificates = () => {
    ...
    // Get full trainee objects for the selected IDs
    const selectedTraineeObjects = trainees.filter(trainee =>
      selectedTrainees.includes(trainee.id)
    );

    try {
      // Try to use sessionStorage instead of localStorage (it persists only for the session)
      sessionStorage.setItem('selectedTrainees', JSON.stringify(selectedTraineeObjects));

      // Create a fallback with URL state in case sessionStorage fails
      const traineesParam = encodeURIComponent(JSON.stringify(selectedTrainees));
      navigate(`/wtc/certificate/preview?trainees=${traineesParam}`);
    } catch (error) {
        ...
        <div className="bg-white shadow rounded-lg overflow-hidden">
          {loading ? (
            ...
            <div className="p-8 text-center text-red-500">
              <p>Error: {error}</p>
              <button onClick={fetchTrainees} className="mt-4 px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600">
                Retry
              </button>
            </div>
            ...
            <div className="p-8 text-center text-gray-600">
              ...
              <button onClick={resetFilters} className="mt-4 px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600">
                Reset Filters
              </button>
            </div>
          ) : (
